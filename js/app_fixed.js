...
async function fetchData(){
  try{
    // Read directly from chrome.storage — no content script needed
    if(typeof chrome==="undefined"||!chrome.storage){
      setStatus("⚠ PIVision Bridge extension not detected",true);
      return;
    }
    const result = await new Promise(resolve=>
      chrome.storage.local.get("pivisionData", r=>resolve(r.pivisionData||null))
    );
    ...
    updateFreezeBar();
    if(!frozen){
      const msg=ageSeconds>60?"⚠ Data "+ageSeconds+"s old — is PIVision tab open?":"LIVE • "+new Date(result._ts).toLocaleTimeString();
      setStatus(msg,true);
      setTimeout(()=>setStatus("",false),1800);
    }
  }catch(err){ console.error(err); setStatus("Bridge error: "+err.message,true); }
}
/* ============================================================
   VALUE HELPERS
   ============================================================ */
